...
export default class {
  ...
  sumarEstado(mensaje) {
    ...
    // Henry: Experimenting with two ways of displaying
    //        conteoPoblaciones: contagious rate for each population, it is the same number with the contador element
    //        estadoActual: for each population, contain two numbers: [% of recuperado, % of (infectado + fallecido)]
    var estadoActual = {};
    var conteoPoblaciones = {};

    this.poblaciones.forEach((nombre) => {
      estadoActual[nombre] = [0, 0];
    });
    ...
    datos.forEach((estado, i) => {
      ...
      if ([2, 3, 4, 5].includes(estado)) {
        for (let nombre of this.poblaciones) {
          const nodo = this.nodos[nombre][0];

          if (nodo.includes(i)) {
            estadoActual[nombre][1] += 1;
            break;
          }
        }
      }
    ...
    this.poblaciones.forEach((nombre) => {
      ...
      estadoActual[nombre][0] = estadoActual[nombre][0] / this.contagiadosPoblaciones[nombre].length;
      estadoActual[nombre][1] = estadoActual[nombre][1] / this.contagiadosPoblaciones[nombre].length;
    ...
    this.linea.actualizar(conteoPoblaciones, estadoActual, this.poblaciones, mensaje.stateI, mensaje.trialI);
  }
  ...
  pintarNodos(nodos) {
    ...
    this.poblaciones.forEach((nombre) => {
      ...
      poblacion.forEach((nodo) => {
        ...
        this.personas[nodo] = new Persona(this.contenedor, nodo, nombre, angulo);
      });
    });
